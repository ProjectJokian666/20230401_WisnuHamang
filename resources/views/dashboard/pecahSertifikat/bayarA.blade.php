...
<div class="container">
    <div class="card shadow">
        <h3 class="card-header">PEMBAYARAN PECAH SERTIFIKAT</h3>
        <div class="card-body">
            <form action="{{url('bayarPecahSertifikatA/'.$data['id'])}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mt-2 ml-5">
                ...
                <div class="mt-5 mb-5 ml-5">
                    ...
                    <a href="{{url('pecahSertifikats')}}" class="btn btn-info col-2">Kembali</a>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="card-body">
            <form>
                <div class="mt-2 ml-5 row">
                    @if(asset('/PECAHSERTIFIKAT/BAYAR/'.$data->bukti))
                    <img class="col-lg-12" src="{{asset('/PECAHSERTIFIKAT/BAYAR/'.$data->bukti)}}" alt="{{$data->bayar}}">
                    @endif
                </div>
            </form>
        </div>
