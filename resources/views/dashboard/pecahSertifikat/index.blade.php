...
<div class="page-breadcrumb">
    <div class="row">
        <div class="col-12 d-flex no-block align-items-center justify-items-center h3">
            ...
            <a href="{{url('pecahSertifikats')}}">{{ __('Pecah Sertifikat') }}</a>
        </div>
    </div>
</div>
<div class="row">
    <div class="col-lg-12">
        <div class="card shadow ">
            <div class="card-body">
                <h3 class=" font-weight-bold text-primary text-center">DATA PECAH SERTIFIKAT</h3>
                    ...
                    <table class="table table-hover" id="dataTable">
                        ...
                        <tbody align="left">
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="{{url('detailPecahSertifikat',$data->id)}}" class="btn btn-sm btn-info">Detail</a>
                                    <a href="{{url('editPecahSertifikat',$data->id)}}" class="btn btn-sm btn-warning">Edit</a>
                                    ...
                                    <a href="{{url('bayarPecahSertifikat',$data->id)}}" class="btn btn-sm btn-success">Pembayaran</a>
                                    @elseif($data->ket == 'dp' && $data->pembayaran == 'sudah')
                                    <a href="{{url('bayarPecahSertifikat',$data->id)}}" class="btn btn-sm btn-success">DP</a>
                                    ...
                                    <a href="{{url('bayarPecahSertifikatA',$data->id)}}" class="btn btn-sm btn-success">Pembayaran</a>
                                    @elseif($data->pembayaran=='sudah' && $data->ket=='dp')
                                    <a href="{{url('bayarPecahSertifikatA',$data->id)}}" class="btn btn-sm btn-success">DP</a>
                                    @elseif($data->pembayaran=='konfirm' && $data->ket=='lunas')
                                    <span class="badge badge-primary p-2">LUNAS</span>
                                    @endif
                                    @endif

                                    <form action="{{url('hapusPecahSertifikat',$data->id)}}" method="post">
                                        @method('delete')
                                        @csrf
    ...
    <div class="modal-dialog modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{url('tambahPecahSertifikat')}}" enctype="multipart/form-data" method="post">
                    @csrf
                    <div class="mb-2">
