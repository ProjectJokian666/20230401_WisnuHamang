    ...
    <div class="card shadow">
        ...
        <div class="card-body">
            <table class="table table-hover" >
                <div class="row mb-5">
                    <div class="col-md-2">
                        <form action="/users/penyertifikatans/batal/{{$datax->id}}" method="post">
                            @method('delete')
                            @csrf
                    ...
                    <div class="col-md-2">
                        <form action="/users/penyertifikatans/selesai/{{$datax->id}}" method="post">
                            @csrf
                ...
                <tbody align="center">
                  <tr>
                    ...
                    <td>{{$datax->fcSurat}}</td>
                    <td><a href="{{asset('Penyertifikatan/Surat/'.$datax->fcSurat)}}" target="blank" class="btn btn-info d-block">Lihat</a></td>
                  </tr>
                <tr>
                <tr>
                <td>Fotocopy KTP Pemohon</td>
                <td>{{$datax->fcKtpPemohon}}</td>
                <td><a href="{{asset('Penyertifikatan/KTP_Pemohon/'.$datax->fcKtpPemohon)}}" target="blank" class="btn btn-info d-block">Lihat</a></td>
              </tr>
              <tr>
                <td>Fotocopy KK Pemohon</td>
                <td>{{$datax->fcKkPemohon}}</td>
                <td><a href="{{asset('Penyertifikatan/KK_Pemohon/'.$datax->fcKkPemohon)}}" target="blank" class="btn btn-info d-block">Lihat</a></td>
              </tr>
              <tr>
                <td>Kematian</td>
                <td>{{$datax->kematian}}</td>
                <td><a href="{{asset('Penyertifikatan/Kematian/'.$datax->kematian)}}" target="blank" class="btn btn-info d-block">Lihat</a></td>
              </tr>
                </tbody>
            </table>
        </div>
